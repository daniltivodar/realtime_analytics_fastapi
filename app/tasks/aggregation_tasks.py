...
@with_async_session
async def _calculate_hourly_aggregation(
    ...
    events = await session.execute(
        ...
        .where(Event.timestamp >= hour_ago)
        .group_by(Event.event_type),
    )
    events_by_type = {row[0]: row[1] for row in events.all()}
...
@celery_app.task
def calculate_hourly_aggregation() -> dict[str, Any]:
...
@celery_task_with_logging(
    ...
    "User behavior calculation failed",
)
@with_async_session
async def _calculate_user_behavior_metrics(
    ...
    if first_event_timestamp is None or last_event_timestamp is None:
        return dict(
            message="Events have no timestamps",
            user_id=user_id,
        )
...
@celery_app.task
def calculate_user_behavior_metrics(user_id: UUID) -> dict[str, Any]:
...
@with_async_session
async def _calculate_daily_summary(session: AsyncSession) -> dict[str, Any]:
    """Async implementation of calculation daily summary."""
    start_of_day = datetime.combine(
        datetime.now(UTC).date() - timedelta(hours=24),
        datetime.min.time(),
    )
...
@celery_app.task
def calculate_daily_summary() -> dict[str, Any]:
