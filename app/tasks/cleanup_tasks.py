...
async def _cleanup_old_redis_data() -> dict[str, Any]:
    ...
    async with redis_service.get_client() as client:
        async for key in client.scan_iter("events:hourly:*", count=100):
            key_date = datetime.strptime(
                str(key).split(":")[-1],
                TIME_FORMAT,
            ).replace(tzinfo=UTC)
            if datetime.now(UTC) - key_date > timedelta(hours=48):
...
@celery_app.task
def cleanup_old_redis_data() -> dict[str, Any]:
    """Cleaning old data from Redis."""
    return asyncio.run(_cleanup_old_redis_data())


@celery_task_with_logging(
    "User sessions cleanup completed",
    "User sessions cleanup failed",
)
async def _cleanup_user_sessions() -> dict[str, Any]:
    """Async implementation of cleanup."""
    deleted_count = 0
    async with redis_service.get_client() as client:
        async for key in client.scan_iter("user:activity:*", count=100):
            ...
            if last_activity:
                ...
                if activity_time < (datetime.now(UTC) - timedelta(days=7)):
                    await client.delete(key)
                    deleted_count += 1
                    logger.debug(
                        "Deleted old user session",
                        extra=dict(key=key),
                    )
    return dict(deleted_count=deleted_count)
...
async def _backup_current_stats() -> dict[str, Any]:
    """Async implementation of backup."""
    stats = await redis_service.get_realtime_stats()
    backup_key = f"backup:stats:{datetime.now(UTC).strftime(TIME_FORMAT)}"
    async with redis_service.get_client() as client:
...
@celery_app.task
def backup_current_stats() -> dict[str, Any]:
